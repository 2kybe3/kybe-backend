...
pub async fn get_trace(ctx: Context<'_>, trace_id: String) -> Result<(), Error> {
    ...
    let mut log = CommandTrace::start(&ctx, "get_trace");
    ...
    if ctx.author().id.to_string() != ctx.data().config.discord_bot.admin_id {
        ...
        log.output = Some("You are not allowed to use this command!".into());
    ...
    let uuid = match Uuid::parse_str(&trace_id) {
        ...
        Err(e) => {
            log.status = CommandStatus::Error;
            log.error = Some(format!("Invalid UUID: {:?}", e));
            log.output = Some("Invalid trace ID format".into());
    ...
    match ctx.data().database.get_command_trace(uuid).await {
        Ok(Some(trace)) => {
            let res = format!("{:#?}", trace);
            let duration = start.elapsed();
            log.output = Some(format!("Trace fetched in {:?}", duration));
            reply_or_attach!(ctx, res, "trace.txt");
        }
        Ok(None) => {
            log.output = Some("No trace found".into());
            ctx.reply(format!("No trace found for ID: `{}`", uuid)).await?;
        }
        Err(e) => {
            log.status = CommandStatus::Error;
            log.error = Some(format!("DB error: {:?}", e));
            log.output = Some(format!("Failed to fetch trace `{}`", uuid));
...
pub async fn get_latest_trace(ctx: Context<'_>) -> Result<(), Error> {
    let start = Instant::now();
    let mut log = CommandTrace::start(&ctx, "get_latest_trace");
    ...
    if ctx.author().id.to_string() != ctx.data().config.discord_bot.admin_id {
        log.status = CommandStatus::Disabled;
        log.output = Some("Not allowed".into());
        ctx.reply("You are not allowed to use this command").await?;
        finalize_command_trace!(ctx, log, start);
        return Ok(());
    }
    ...
    match ctx.data().database.get_latest_command_trace().await {
        Ok(Some(trace)) => {
            ...
            let duration = start.elapsed();
            log.output = Some(format!("Latest Trace fetched in {:?}", duration));
            reply_or_attach!(ctx, output, "latest_trace.txt");
        }
        Ok(None) => {
            log.output = Some("No traces found".into());
            ctx.reply("No traces found").await?;
        }
        Err(e) => {
            log.status = CommandStatus::Error;
            log.error = Some(format!("{:?}", e));
            log.output = Some("DB error".into());
